...
describe("VisualMassageModule", () => {
  ...
  describe("Contract Data", async function () {
    ...
    it("can change contract baseURI if owner of contract", async function () {
      ...
      await expect(await visualMassageModule.baseURI()).to.be.equal(newBaseURI);
    });
  ...
  describe("Helixes", async function () {
    it("can change full helix", async function () {
      ...
      expect(helix).to.not.be.deep.equal(newHelix);

      await visualMassageModule.configureHelix(
        1,
        newHelix.startId,
        newHelix.endId,
        newHelix.price,
    ...
    it("can change helix price", async function () {
      ...
      const helix = await visualMassageModule.helixes(1);
      expect(helix.price).to.not.be.equal(newPrice);

      await visualMassageModule.setHelixPrice(1, newPrice);
      expect(
        await visualMassageModule.helixes(1).then((helix) => helix.price)
      ).to.be.equal(newPrice);
    });

    it("can change helix ids", async function () {
      const helix = await visualMassageModule.helixes(1);
      expect([helix.startId, helix.endId]).to.not.be.equal([10, 30]);

      await visualMassageModule.setHelixIds(1, 10, 30);
      expect(
        await visualMassageModule
          .helixes(1)
          .then((helix) => [helix.startId, helix.endId])
      ).to.be.deep.equal([10, 30]);
    });

    it("can change helix signer", async function () {
      const helix = await visualMassageModule.helixes(1);
      expect(helix.signer).to.not.be.equal(signer.address);

      await visualMassageModule.setHelixSigner(1, signer.address);
      expect(
        await visualMassageModule.helixes(1).then((helix) => helix.signer)
      ).to.be.deep.equal(signer.address);
    });
  });
  ...
  describe("Minting", async function () {
    it("can mint with signature", async function () {
      ...
      await visualMassageModule
        .connect(random)
        .collectWithSignature(110, nonce, signature);
    });
    ...
    it("can not mint with wrong signature", async function () {
      ...
      await expect(
        visualMassageModule.connect(random).collectWithSignature(
          110,
    ...
    it("can mint", async function () {
      ...
      expect(await nftContract.ownerOf(1)).to.be.equal(
    ...
    it("can not mint with wrong value", async function () {
      ...
      await visualMassageModule.setCollectActive(true);
      await expect(
        visualMassageModule.collect(1, { value: 0 })
  ...
  describe("Withdraw", async () => {
    it("withdrawing works", async function () {
      ...
      expect(
        shares[0].add(shares[1]).add(shares[2]).add(shares[3])
      ).to.be.equal(balance);

      const accounts = [];
      for (const recipient of recipients) {
        accounts.push(new ethers.VoidSigner(recipient, deployer.provider));
      }
